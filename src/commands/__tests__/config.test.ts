...
describe('config command', () => {
  ...
  describe('set subcommand', () => {
    ...
    it('should exit with error when key is missing', () => {
      expect(() => handleConfigCommand('set', [], {})).toThrow('process.exit(1)');
      expect(processExitSpy).toHaveBeenCalledWith(1);
    });

    it('should exit with error when value is missing', () => {
      expect(() => handleConfigCommand('set', ['apiToken'], {})).toThrow('process.exit(1)');
      expect(processExitSpy).toHaveBeenCalledWith(1);
    });

    it('should exit with error for invalid configuration key', () => {
      expect(() => handleConfigCommand('set', ['invalidKey', 'value'], {})).toThrow('process.exit(1)');
      expect(processExitSpy).toHaveBeenCalledWith(1);
  ...
  describe('get subcommand', () => {
    ...
    it('should exit with error for invalid configuration key', () => {
      expect(() => handleConfigCommand('get', ['invalidKey'], {})).toThrow('process.exit(1)');
      expect(processExitSpy).toHaveBeenCalledWith(1);
    });
  ...
  describe('validate subcommand', () => {
    ...
    it('should exit with error when configuration is invalid', () => {
      ...
      expect(() => handleConfigCommand('validate', [], {})).toThrow('process.exit(1)');
      expect(configModule.validateConfig).toHaveBeenCalled();
      expect(processExitSpy).toHaveBeenCalledWith(1);
    });
    ...
    it('should validate configuration in json format', () => {
      ...
      expect(() => handleConfigCommand('validate', [], { format: 'json' })).toThrow('process.exit(0)');
      expect(configModule.validateConfig).toHaveBeenCalled();
    ...
    it('should validate configuration in json format with errors', () => {
      ...
      expect(() => handleConfigCommand('validate', [], { format: 'json' })).toThrow('process.exit(1)');
      expect(configModule.validateConfig).toHaveBeenCalled();
      expect(processExitSpy).toHaveBeenCalledWith(1);
    });
  });
