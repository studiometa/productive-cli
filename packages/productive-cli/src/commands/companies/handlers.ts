...
export async function companiesUpdate(args: string[], ctx: CommandContext): Promise<void> {
  ...
  await runCommand(async () => {
    ...
    if (ctx.options["billing-name"] !== undefined) data.billing_name = String(ctx.options["billing-name"]);
    ...
    if (ctx.options["due-days"] !== undefined) data.due_days = parseInt(String(ctx.options["due-days"]));
