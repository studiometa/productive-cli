...
export class HumanDealListRenderer implements ListRenderer<FormattedDeal> {
  render(
    data: FormattedListResponse<FormattedDeal>,
    ctx: RenderContext,
  ): void {
    ...
    for (const deal of deals) {
      ...
      const typeColored = deal.type === "budget"
        ? ctx.noColor ? typeBadge : colors.blue(typeBadge)
        : ctx.noColor ? typeBadge : colors.green(typeBadge);
      ...
      console.log(`${typeColored} ${ctx.noColor ? numberStr + deal.name : colors.bold(numberStr + deal.name)}${statusBadge}`);
      ...
      // Dates
      if (deal.date) {
        const dateInfo = deal.end_date
          ? `${deal.date} → ${deal.end_date}`
          : `Starts: ${deal.date}`;
...
export class HumanDealDetailRenderer implements Renderer<FormattedDeal> {
  render(deal: FormattedDeal, ctx: RenderContext): void {
    ...
    // Number and name
    const numberStr = deal.number ? `#${deal.number} ` : "";
    console.log(`${typeColored} ${ctx.noColor ? numberStr + deal.name : colors.bold(numberStr + deal.name)}`);
